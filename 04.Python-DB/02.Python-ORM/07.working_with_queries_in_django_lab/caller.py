...
def delete_review_by_id(id: int) -> str or None:
...
def filter_authors_by_nationalities(nationality: str or None) -> str:
    authors = Author.objects.filter(nationality=nationality).order_by('first_name', 'last_name')

    return '\n'.join(f'{a.biography}'if a.biography else a.__str__() for a in authors)


def filter_authors_by_birth_year(year_from: int, year_to: int) -> str:
    authors = Author.objects.filter(birth_date__year__range=(year_from, year_to)).order_by('-birth_date')

    return '\n'.join(f'{a.birth_date}: {a.__str__()}' for a in authors)
