...
class SummitQuestManagerApp:
    ...
    def register_climber(self, climber_type: str, climber_name: str) -> str:
        ...
        try:
            next(filter(lambda x: x.name == climber_name, self.climbers))
    ...
    def check_gear(self, climber_name: str, peak_name: str, gear: List[str]) -> str:
        ...
        if gear != peak.get_recommended_gear():
            ...
            return (f"{climber_name} is not prepared to climb {peak_name}. "
                    f"Missing gear: {', '.join(sorted(g for g in peak.get_recommended_gear() if g not in gear))}.")
    ...
    def get_statistics(self) -> str:
        successfull_climbers = filter(lambda c: len(c.conquered_peaks) > 0, self.climbers)
        sorted_climbers = sorted(successfull_climbers, key=lambda c: (-len(c.conquered_peaks), c.name))

        climbed_peaks = len(set(p for c in sorted_climbers for p in c.conquered_peaks))

        return (f"Total climbed peaks: {climbed_peaks}\n" +
